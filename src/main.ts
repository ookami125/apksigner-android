    ...
    if (releaseFiles !== undefined && releaseFiles.length !== 0) {
      ...
      for (let releaseFile of releaseFiles) {
        ...
        if (releaseFile.name.endsWith('.apk')) {
          signedReleaseFile = await signApkFile(releaseFilePath, signingKey, alias, keyStorePassword, keyPassword, customArgs);
        } else if (releaseFile.name.endsWith('.aab')) {
          signedReleaseFile = await signAabFile(releaseFilePath, signingKey, alias, keyStorePassword, keyPassword, customArgs);
        } else {
